...
|`insertBefore`| `Number`                                                     | Монтировать в `parent` перед индексом. Работает также для `child`                                                                           |
| `tag`        | `Строка`                                                     | HTML тег для child-объектов                                                                                                                 |
| `svg`        | `Boolean`                                                    | Флаг SVG для child-объектов                                                                                                                 |
| `children`   | `Конфиг\|Массив [Конфиг\|HTML\|Node\|Null\|Массив\|State]`   | Создать детей и добавить к элементу. Без указания `tag` будет добавлен `div`. Null игнорируются                                             |
...
>- `State`-бинды на тот же параметр (+ под-параметр) и имя заменяют старые
>- Обработчики событий элементов на те же события заменяют старые. С параметром `once: true` - не заменяют
...
EL.mount(el, parent, beforeIndex = null);
